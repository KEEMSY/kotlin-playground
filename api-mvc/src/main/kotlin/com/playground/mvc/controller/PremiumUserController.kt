...
import com.playground.mvc.service.PremiumUserService
...
class PremiumUserController(
    ...
    @Operation(
        ...
        description = """
            ...
            ## Fallback 시나리오
            - Circuit Breaker OPEN: PENDING 반환
            - 결제 오류: PENDING 반환
        """
    )
    @PostMapping("/with-fallback")
    fun upgradeToPremiumWithFallback(
        @Valid @RequestBody request: CreatePremiumUserRequest
    ): ResponseEntity<PremiumUserResponse> {
    ...
    @Operation(
        ...
        description = """
            ...
            ## 메트릭
            - failureRate: 실패율 (%)
            - numberOfFailedCalls: 실패 횟수
            - numberOfSuccessfulCalls: 성공 횟수
        """
    )
